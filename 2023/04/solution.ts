...
const findAnswers = (entries: string[][], isLog = true) => {
  ...
  answers.a = input
    .map((card) => {
      const matches = card.numbers.filter((n) => card.winning.includes(n));
      return matches.length > 0 ? matches.length : 0;
    })
    .reduce((a, b) => {
      if (b == 0) return a;
      return a + Math.pow(2, b - 1);
    }, 0);
  ...
  let cards = 0
  const multiplier = new Array(wins.length).fill(1);
...
const testPart2 = async (input: string): Promise<boolean> => {
  ...
  return answers.b == 30 ? true : false;
};
